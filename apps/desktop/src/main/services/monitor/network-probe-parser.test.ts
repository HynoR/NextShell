...
(() => {
  ...
  assertEqual(
    parsed.listeners[0]?.connectionCount,
    1,
    "netstat parser should aggregate connection count"
  );
...
(() => {
  ...
  assertEqual(
    parsed.listeners[0]?.name,
    "unknown",
    "missing process name should fallback to unknown"
  );
  ...
  assertEqual(
    parsed.connections[0]?.processName,
    "unknown",
    "missing process name should fallback to unknown"
  );
  assertTrue(
    parsed.connections[0]?.remotePort === 51666,
    "connection remote port should still be parsed"
  );
})();
